...
namespace NES {
  ...
  void CPU::writeVRAM(Address addr, Byte num) {
    // cout << "writeVRAMbefore(" << hex << setw(4) << setfill('0') << addr <<
  ...
  std::vector<Byte> CPU::getBackGroundColor(Byte data) {
  ...
  std::vector<Byte> CPU::getSpriteColor(Byte data) {
    std::vector<Byte> res(4);
  ...
  std::vector<std::vector<std::vector<int>>> CPU::setPixcels() {
    /*------------------Render BackGround------------------------*/
    ...
    for(int i = 0; i < 30; i++) {
      for(int j = 0; j < 32; j++) {
        ...
        std::vector<Byte> sprite = CharacterRom[nameTable[i][j]];
    ...
    for(int i = 0; i < 256; i += 4) {
      ...
      std::vector<Byte> sprite = CharacterRom[spriteram[i + 1]];
